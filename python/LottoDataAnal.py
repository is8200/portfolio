...
class LottoDataAnal :
    ...
    def connection(self) :
        ...
        lottoRDD = lottoBasicDatasRDD.map(dataToDetailFunction) \
           ...
           #.partitionBy(4) \
           # .cache()
        #lottoDF = SparkSession.createDataFrame(lottoRDD)
        #checkValue = self.checkValue
        #lottoRDD.foreach(checkValue)        
        abc = lottoRDD.collect()
        print(abc)
        #scSession = SparkSession.builder.getOrCreate()
        dfRDD = lottoRDD.map(lambda data : Row(name='xxx', no=data[2]))
        sqlCtx = SQLContext(sc)
        df = sqlCtx.createDataFrame(dfRDD)
        #df = SparkSession.createDataFrame(self, data=abc)
        print("xxxx")
        df.show()
        pdf = df.toPandas()
