...
object SparkBookStreamingAdvancedWithJDBCServerSum {
  def main(args: Array[String]) {

    if(args.length != 2) {
      System.err.println(
        "Usage: SparkBookStreamingAdvancedWithJDBCServerSum <host> <port>")
      System.exit(1)
    }

    val sparkConf = new SparkConf().setAppName("SparkBookStreamingAdvancedWithJDBCServerSum")
    ...
    predictionStringDS.countByValue().foreachRDD{ rdd =>
      val behaviorCountDF = rdd.toDF("behavior", "count")
      behaviorCountDF.registerTempTable("behavior_count")
    }
